...
export function Certifications({ items }: CertificationsProps) {
  ...
  return (
    <section id="certifications" className="w-full py-20 md:py-32 px-6 md:px-12 scroll-mt-20" style={{ background: "#182240" }}>
          ...
          {sortedItems.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className="group rounded-xl border border-[#3D4F6B] p-6 card-hover-glow relative overflow-hidden"
              ...
              >
                ...
                <div className="flex justify-between items-start mb-5">
                  <div
                    className="p-2.5 rounded-lg border border-[#3D4F6B]"
                    style={{ background: "#182240" }}
                  >
                    {/* Shield-check icon in monochrome */}
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-5 h-5 text-[#CBD5E1]" aria-hidden="true">
                  ...
                  <span
                    className={`font-mono text-[10px] uppercase tracking-widest px-2 py-1 rounded ${
                      cert.active
                        ...
                        : "text-[#CBD5E1] border border-[#3D4F6B]"
                    }`}
                ...
                <p className="font-mono text-xs text-[#CBD5E1] mb-5">
                  {cert.issuer}
                </p>

                <div className="flex items-center justify-between pt-4 border-t border-[#3D4F6B]">
                  <span className="font-mono text-[11px] text-[#CBD5E1]">{cert.date}</span>
                  <div className="flex gap-3">
                    ...
                    {cert.imageUrl && (
                      <button
                        ...
                        className="font-mono text-[11px] text-[#CBD5E1] hover:text-[#F1F5F9] transition-colors"
                      >
                        View
                      </button>
                    )}
      ...
      {selectedImage && (
        ...
        >
          <div className="relative max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute -top-12 right-0 p-2 text-[#CBD5E1] hover:text-[#F1F5F9] transition-colors"
            >
              <X className="w-6 h-6" />
            </button>
            <img
